...
After the initial Desktop check and a matching CLI login, one shared worker checks quota every 60 seconds while any enabled Pet shows quota. The rate stays the same when its task is idle, ended, or waiting for a reply, because other conversations can consume the same account allowance. Intervals run from a successful read; failures back off. All quota displays hidden, all Pets disabled, or app quit stop polling and cancel pending work. These are direct account reads, not repeated model prompts.
...
The current source checks include 260 Python tests and 96 JavaScript state tests. The visibility-based quota policy was checked on the development Mac with only an ended task showing quota while work continued in a task without an enabled Pet: two successful automatic reads were 60.67 seconds apart. The percentage stayed the same while the actual observation time advanced, and the native UI showed the new 60-second policy. Other machines, older macOS releases, Intel builds, and a prebuilt distribution remain unverified.
