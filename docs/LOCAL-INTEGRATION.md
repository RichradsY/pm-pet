...
One shared worker serves all visible quota copies. With an enabled Pet and at least one visible quota display, a successful read schedules the next check after 60 seconds, regardless of the bound tasks' activity or pending questions. Account usage can change through other conversations, including ones without a Pet, so this schedule does not infer account activity from a task or scan other conversations. Requests do not overlap. Hiding every quota display or disabling every Pet stops polling and cancels an in-flight read; progress observation remains separate. There is no Pet manual-refresh button or OS-wake hook.
