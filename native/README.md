...
No quota values or progress metrics are seeded by the native UI. Missing five-hour quota remains absent. Missing quota is shown as unavailable when its display is enabled. Quota retains its actual observation timestamp and shows its source and relative age outside the glass pill. Values older than five minutes are marked cached with an asterisk; the visible renderer updates age every 30 seconds without fetching data.

A genuine Desktop usage check seeds conditional automatic reads through the bridge. With a matching file-based CLI account and at least one enabled Pet showing quota, one shared worker schedules the next direct read after 60 seconds. The interval is independent of task activity and pending questions, so an idle Pet still receives account usage consumed by other conversations. All quota hidden or all Pets disabled cancels polling. The compact line shows Auto, Checking, Retry, or Account check needed; a successful read updates the timestamp even if its percentage is unchanged. The CLI match is to the last Desktop account check, not a permanent live Desktop identity. See [quota sources and automatic refresh](../docs/LOCAL-INTEGRATION.md#quota-sources-and-automatic-refresh) for validation, failure handling, and the separate unverified diagnostic probe.
