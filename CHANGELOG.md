...
- Refresh shared account quota every 60 seconds while any enabled Pet shows it, including when the bound task is idle, ended, or awaiting a reply. This replaces task-dependent 60/300-second timing, which missed consumption in conversations without a Pet. Retain one worker, visibility-based cancellation, account verification, and failure backoff; synchronize native/prototype copy and installation guidance.
- Clarify that progress measures the reported roadmap, which may span multiple turns. Explain remaining work after a turn ends or is interrupted, and label gray dots as not marked complete instead of assuming they were never started.
- Keep plan updates ordered independently from turn activity so a delayed plan is not discarded just because a newer turn-ending event arrived first. Preserve newer reports, request boundaries, and pending-question protections, and synchronize the current step with each accepted plan.
...
- Share one quota worker across Pets. Hide-all and disable-all cancel polling; failures back off with truthful cached age, and account mismatch shows unavailable. Add Auto, Checking, Retry, and Account check needed states. Automatic updates require an initial Desktop usage check and an optional installed CLI with matching ChatGPT file authentication; the standalone probe remains unverified.
