...
**After enabling Pet, ask Codex to check current usage once.** Pet observes the genuine Desktop result and verifies that its local CLI account matches that last Desktop account check. When that check succeeds and at least one enabled Pet shows quota, one shared background worker reads usage directly **every 60 seconds**, scheduled after a successful read. This continues when the Pet's task is idle, ended, or waiting for your reply: other conversations can still consume the same account allowance. Several quota displays share the same read. Refreshes do not send model prompts. Hiding all quota displays or disabling all Pets stops polling and cancels an in-flight read.
