...
Quota comes from general Codex snapshots or completed Desktop `get_usage_limits` results in an enabled bound conversation, with their real timestamps. When the user asks to check or refresh usage, use the Desktop usage-limits tool; Pet observes its successful result without a manual quota report. Old figures are marked cached after five minutes. This does not implement automatic account polling. A separate CLI-account quota probe is available for diagnostics, but its account has not been proven to match the desktop account. Never relabel cached data as a fresh API read.
