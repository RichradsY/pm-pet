...
- After successful enable, use the Codex Desktop `get_usage_limits` tool once in that enabled root task to initialize quota account verification. If the tool or a matching file-based CLI login is unavailable, explain that automatic quota updates are unavailable and continue the build; do not create a blocking setup loop. Do not install or change CLI authentication without a separate user request. The background reader handles later checks without repeated model prompts.
...
Quota comes from general Codex snapshots, completed Desktop `get_usage_limits` results, and conditional direct CLI account reads. The first genuine Desktop result seeds a fingerprint: the automatic reader requires matching ChatGPT file-login metadata and unchanged auth-file generation before and after the read. Each child uses that same `CODEX_HOME` and file credential store. This verifies the CLI against the last Desktop account check, not a permanent live Desktop identity; keychain-only or unmatched credentials cannot enable automatic reads.

One shared worker schedules the next direct read 60 seconds after success while any enabled Pet shows quota, including when its task is idle, ended, or awaiting a reply. Other conversations can consume the same account allowance, so the schedule does not depend on bound-task activity or require observing unbound conversations. It uses account APIs, not model prompts. All quota displays hidden or all Pets disabled cancels polling. Failures back off with actual cached age; account mismatch shows unavailable. When requested, or when Account check needed appears, use the Desktop usage-limits tool again; no manual Pet quota report is needed. Do not copy raw account IDs, tokens, credit balances, or answers into Pet reports.

Auto, Checking, Retry, and Account check needed describe source or refresh status. Old figures are marked cached after five minutes. A successful read can return an unchanged percentage with a new actual timestamp; rereading cached data is never a fresh API read. The standalone CLI `--probe` remains an unverified diagnostic and cannot substitute for the Desktop account check or seed the automatic worker.
